...
def MailSender(filename,time):
    try:
        fromaddr = "shubhamlasurve77@gmail.com"
        toaddr = "svlasurve@gmail.com"

        msg = MIMEMultipart()       #msg ha MIMEMultipart() cha object ahe
        ...
        msg.attach(MIMEText(body, 'plain'))     #parameter body pathvnar ani to text plain ahe
    
        attachment = open(filename, "rb")       #mail send sathi attacchment hi binary madhech pahij 
    
        p = MIMEBase('application', 'octet-stream')
    
        p.set_payload((attachment).read())     
    
        encoders.encode_base64(p)       #encoding algorithm ahe

        p.add_header('Content-Disposition', "attachment; filename= %s" % filename)      
    
        msg.attach(p)       #file attach keli
    
        s = smtplib.SMTP('smtp.gmail.com', 587)
    
        s.starttls()
    
        s.login(fromaddr,"xsrf kyku qssq ywcu")
...
def ProcessLog(log_dir = 'Marvellous'):
    ...
    connected = is_connected()      #Internet connection ahe ki nahi ya sathi
